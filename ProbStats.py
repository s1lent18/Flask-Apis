...
def Taylor():
    try:
        lb = request.args.get('lb', type=float)
        ub = request.args.get('ub', type=float)
        eq = request.args.get('eq', type=str)
        o = request.args.get('o', type=float)
        deg = request.args.get('deg', type=int)
        ev = request.args.get('ev', type=float)
        
        x = sp.symbols('x')
        f = sp.sympify(eq, locals={'x': x})
        f = sp.sympify(f, locals={'x': x})
        
        P = 0
        f_derivative = f
        for n in range(deg + 1):
            f_n = f_derivative.subs(x, o)
            P += (f_n / sp.factorial(n)) * (x - o)**n
            f_derivative = sp.diff(f_derivative, x)
        
        P = sp.collect(P, x - o)
        
        P_at_eval = P.subs(x, ev)
        f_at_eval = f.subs(x, ev)
        actual_error = abs(f_at_eval - P_at_eval)

        f_quad_prime = sp.diff(f_derivative, x)
        f_quad_prime_max = max(abs(f_quad_prime.subs(x, ev)), abs(f_quad_prime.subs(x, o)))
        error_bound = f_quad_prime_max / sp.factorial(deg + 1) * (ev - o)**(deg + 1)

        if lb and ub:
            integral_P = sp.integrate(P, (x, lb, ub))
            integral_f = sp.integrate(f, (x, lb, ub))
            actual_integral_error = abs(integral_f - integral_P)
            
            error_bound_integral = sp.integrate(f_quad_prime_max / sp.factorial(deg + 1) * (x - o)**(deg + 1), (x, lb, ub))
            
        P_at_eval = round(P_at_eval, 5)
        f_at_eval = round(f_at_eval, 5)
        actual_error = round(actual_error, 5)
        error_bound = round(error_bound, 5)
        integral_P = round(integral_P, 5)
        integral_f = round(integral_f, 5)
        actual_integral_error = round(actual_integral_error, 5)
        error_bound_integral = round(error_bound_integral, 5)

        return jsonify({
            'taylor_polynomial': str(P),
            'p_at_ev': float(P_at_eval),
            'f_at_ev': float(f_at_eval),
            'actual_error': float(actual_error),
            'error_bound': float(error_bound),
            'integral_of_p': float(integral_P),
            'integral_of_f': float(integral_f),
            'actual_integral_error': float(actual_integral_error),
            'integral_error_bound': float(error_bound_integral)
        })
